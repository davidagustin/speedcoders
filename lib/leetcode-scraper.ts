...
const ALGORITHM_PATTERNS: AlgorithmPattern[] = [
  ...
  {
    name: 'Binary Search',
    patterns: ['binary search', 'mid.*point', 'left.*right.*mid', 'search space'],
    keywords: ['sorted', 'rotated', 'peak', 'minimum', 'maximum'],
    complexities: { time: ['O(log n)'], space: ['O(1)', 'O(log n)'] }
  },
  {
    name: 'Dynamic Programming',
    patterns: ['dp', 'memoization', 'tabulation', 'subproblem', 'optimal substructure'],
    keywords: ['fibonacci', 'climbing stairs', 'knapsack', 'longest', 'minimum cost'],
  ...
  },
  {
    ...
    patterns: ['dfs', 'recursion', 'backtrack', 'preorder', 'inorder', 'postorder'],
  ...
  {
    name: 'Backtracking',
    patterns: ['backtrack', 'explore.*paths', 'generate.*combinations', 'permutation'],
...
export function detectAlgorithmsFromProblem(problem: LeetCodeProblem): string[] {
  const detectedAlgorithms = new Set<string>()
  const content = `${problem.description} ${problem.solutions.map(s => s.approach).join(' ')}`.toLowerCase()
  ...
  for (const algo of ALGORITHM_PATTERNS) {
    ...
    for (const solution of problem.solutions) {
      if (algo.complexities.time.includes(solution.complexity.time) ||
...
export async function fetchLeetCodeProblem(slug: string): Promise<LeetCodeProblem | null> {
  ...
  if (slug === 'two-sum') {
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        {
          approach: 'Hash Table: Use a hash map to store complements for O(1) lookup',
...
export async function saveProblemToDatabase(problem: LeetCodeProblem, slug: string) {
...
export async function createQuizFromProblems(problemSlugs: string[], title: string, timeLimit: number = 30) {
  const supabase = await createClient()
  ...
  for (let i = 0; i < problemSlugs.length; i++) {
    ...
    if (problem) {
      const savedProblem = await saveProblemToDatabase(problem, problemSlugs[i])
