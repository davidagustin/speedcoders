...
export class QuizManager {
  ...
  async createQuiz(quiz: Omit<Quiz, 'id' | 'attempts' | 'averageScore' | 'createdAt' | 'updatedAt'>): Promise<Quiz> {
    try {
  ...
  async getQuizzes(filters: {
    ...
    try {
      ...
      const cached = await this.cache.get<{ quizzes: Quiz[], total: number }>(cacheKey)
      if (cached) return cached
  ...
  async startQuizAttempt(quizId: string, userId: string): Promise<{ attemptId: string, quiz: Quiz } | null> {
    try {
      // Check rate limit
      const rateLimitKey = `quiz_start:${userId}`
      const { allowed } = await this.rateLimiter.checkLimit(rateLimitKey)
      if (!allowed) {
        throw new Error('Rate limit exceeded. Please wait before starting another quiz.')
      }
      ...
      // Cache the attempt
      await this.cache.set(`attempt:${data.id}`, {
        id: data.id,
        quizId,
        userId,
  ...
  async submitQuizAttempt(
    ...
    try {
      ...
      const attempt = await this.cache.get(`attempt:${attemptId}`)
  ...
  async generateQuiz(options: {
    ...
    try {
      ...
      if (!problems || problems.length < options.questionCount) {
        throw new Error('Not enough problems available for the specified criteria')
      }

      // Randomly select problems
      const selectedProblems = this.shuffleArray(problems).slice(0, options.questionCount)

      // Generate questions from problems
      const questions: QuizQuestion[] = selectedProblems.map((problem, index) => ({
        id: `q${index + 1}`,
  ...
  private calculateScore(quiz: Quiz, answers: Record<string, number[]>): { score: number, feedback: QuizFeedback[] } {
    let correctCount = 0
    const feedback: QuizFeedback[] = []

    quiz.questions.forEach(question => {
      const userAnswer = answers[question.id] || []
      const correct = this.arraysEqual(userAnswer.sort(), question.correctAnswers.sort())
  ...
  private async updateQuizStats(quizId: string, score: number): Promise<void> {
    try {
      ...
      const newAverageScore = ((data.average_score || 0) * (data.attempts || 0) + score) / newAttempts
