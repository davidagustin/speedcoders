...
export class AdaptiveLearningEngine {
  ...
  async generateUserProfile(userId: string): Promise<UserLearningProfile> {
    ...
    return {
      ...
      lastActiveDate: attempts[0]?.createdAt || new Date(),
      learningGoals: await this.inferLearningGoals(userId, algorithmPerformance)
    };
  }

  async createPersonalizedLearningPath(userId: string, goals: string[] = []): Promise<LearningPath> {
    ...
    const problems = await this.selectOptimalProblems(profile, recommendations, goals);
    ...
    const learningPath: LearningPath = {
      ...
      estimatedDuration: this.estimateCompletionTime(problems, profile.learningVelocity),
  ...
  async adaptLearningPath(pathId: string): Promise<LearningPath> {
    ...
    const recentPerformance = await this.analyzeRecentPerformance(path.userId, 7); // Last 7 days
    
    // Check if adaptation is needed
    const adaptationNeeds = this.assessAdaptationNeeds(path, profile, recentPerformance);
    ...
    // Apply adaptations
    const adaptedPath = await this.applyAdaptations(path, adaptationNeeds, profile);
  ...
  async generateMLRecommendations(profile: UserLearningProfile): Promise<MLRecommendation[]> {
  ...
  private analyzeAlgorithmPerformance(attempts: any[]): Record<string, number> {
    ...
    attempts.forEach(attempt => {
      attempt.quiz.questions.forEach((question: any) => {
        const problem = comprehensiveProblems.find(p => p.title === question.problem.title);
        if (problem) {
          problem.algorithms.forEach(algo => {
            ...
            if (attempt.score >= 70) { // Consider 70%+ as correct
  ...
  private analyzeTimeSpent(attempts: any[]): { byCategory: Record<string, number>; average: number } {
  ...
  private analyzeDifficultyPreference(attempts: any[]): { preferred: 'Easy' | 'Medium' | 'Hard'; distribution: Record<string, number> } {
    const difficultyScores: Record<string, number[]> = { Easy: [], Medium: [], Hard: [] };
    ...
    Object.entries(difficultyScores).forEach(([diff, scores]) => {
      avgScores[diff] = scores.length > 0 ? scores.reduce((a, b) => a + b, 0) / scores.length : 0;
    });

    // Find difficulty with best performance (above 75% accuracy)
    const preferred = (Object.entries(avgScores)
      .filter(([_, score]) => score >= 75)
      .sort(([_, a], [__, b]) => b - a)[0]?.[0] as 'Easy' | 'Medium' | 'Hard') || 'Easy';
  ...
  private calculateLearningVelocity(attempts: any[]): number {
    ...
    const timeSpan = new Date(sortedAttempts[sortedAttempts.length - 1].createdAt).getTime() - 
  ...
  private identifyStrengths(algorithmMastery: Record<string, number>): string[] {
  ...
  private identifyWeaknesses(algorithmMastery: Record<string, number>): string[] {
    return Object.entries(algorithmMastery)
      .filter(([_, mastery]) => mastery < this.MASTERY_THRESHOLD)
      .sort(([_, a], [__, b]) => a - b)
      .slice(0, 5)
  ...
  private async inferLearningGoals(userId: string, algorithmMastery: Record<string, number>): Promise<string[]> {
    const goals: string[] = [];
    ...
    const overallMastery = Object.values(algorithmMastery).reduce((a, b) => a + b, 0) / Object.keys(algorithmMastery).length;
  ...
  private createBasicProfile(userId: string): UserLearningProfile {
    return {
      ...
      learningGoals: ['Build algorithmic thinking skills', 'Establish consistent practice']
    };
  }

  private calculateAverageAccuracy(attempts: any[]): number {
    if (attempts.length === 0) return 0;
    const totalAccuracy = attempts.reduce((sum, attempt) => sum + attempt.score, 0);
  ...
  private async selectOptimalProblems(
    ...
    const targetAlgorithms = new Set([...profile.weaknesses, ...this.extractAlgorithmsFromGoals(goals)]);
    ...
    foundationalProblems.forEach((problem, index) => {
      problems.push(this.createRecommendedProblem(problem, index + 1, 8, 
    ...
    const progressiveProblems = comprehensiveProblems
      .filter(p => p.difficulty === profile.preferredDifficulty &&
                   p.algorithms.some(algo => targetAlgorithms.has(algo)))
      .slice(0, 8);
    
    progressiveProblems.forEach((problem, index) => {
      problems.push(this.createRecommendedProblem(problem, foundationalProblems.length + index + 1, 7,
    ...
    if (profile.strengths.length > 0) {
      ...
      challengeProblems.forEach((problem, index) => {
        problems.push(this.createRecommendedProblem(problem, problems.length + index + 1, 6,
          'Challenge problems to maintain strength areas'));
      });
    }

    return problems.sort((a, b) => b.priority - a.priority || a.order - b.order);
  }

  private createRecommendedProblem(problem: any, order: number, priority: number, reasoning: string): RecommendedProblem {
    return {
  ...
  private estimateProblemTime(problem: any): number {
    ...
    return Math.round((baseTime[problem.difficulty as keyof typeof baseTime] || 30) * complexityMultiplier);
  }
  ...
  private generateLearningObjectives(problem: any): string[] {
    ...
    objectives.push(`Understand ${problem.difficulty.toLowerCase()} level problem patterns`);
  ...
  private extractAlgorithmsFromGoals(goals: string[]): string[] {
    ...
    const algorithmKeywords = [
      'Array', 'String', 'Dynamic Programming', 'Graph', 'Tree', 'Binary Search',
      'Sorting', 'Hash Table', 'Stack', 'Queue', 'Heap', 'Backtracking'
    ];
  ...
  private recommendOptimalDifficulty(profile: UserLearningProfile): MLRecommendation {
    ...
    if (avgAccuracy >= 85 && profile.consistencyScore >= 70) {
      ...
      recommendedDifficulty = difficultyMap[profile.preferredDifficulty as keyof typeof difficultyMap] as any;
      confidence = 0.85;
      reasoning = 'High accuracy and consistency indicate readiness for increased difficulty';
      ...
      recommendedDifficulty = difficultyMap[profile.preferredDifficulty as keyof typeof difficultyMap] as any;
      confidence = 0.8;
      reasoning = 'Low accuracy or consistency suggest focusing on foundational concepts';
    }
  ...
  private recommendTopicsToFocus(profile: UserLearningProfile): MLRecommendation[] {
    const recommendations: MLRecommendation[] = [];
    ...
    profile.weaknesses.slice(0, 3).forEach(weakness => {
      recommendations.push({
        ...
        data: { topic: weakness, currentMastery: profile.algorithmMastery[weakness] || 0 },
  ...
  private recommendProblemSequence(profile: UserLearningProfile): MLRecommendation {
    let sequence = 'mixed';
    let confidence = 0.6;
    ...
    if (profile.consistencyScore < 50) {
      sequence = 'structured';
      confidence = 0.8;
      reasoning = 'Low consistency benefits from structured, sequential learning';
    } else if (profile.totalProblemsAttempted > 50 && profile.averageAccuracy > 75) {
      sequence = 'adaptive';
      confidence = 0.85;
      reasoning = 'Experienced user with good performance can handle adaptive sequencing';
    }
  ...
  private recommendOptimalPacing(profile: UserLearningProfile): MLRecommendation {
    ...
    if (profile.consistencyScore >= 80 && profile.averageAccuracy >= 75) {
      ...
      confidence = 0.85;
      reasoning = 'Inconsistent performance suggests slowing down for better retention';
    }
    ...
    return {
      ...
      expectedImprovement: Math.abs(recommendedPacing - currentVelocity) > 1 ? 15 : 5
    };
  }

  private generatePathTitle(profile: UserLearningProfile, goals: string[]): string {
    if (goals.length > 0) {
  ...
  private generatePathDescription(profile: UserLearningProfile, goals: string[]): string {
    const weaknessText = profile.weaknesses.length > 0 
  ...
  private estimateCompletionTime(problems: RecommendedProblem[], velocity: number): number {
  ...
  private determinePathDifficulty(profile: UserLearningProfile, problems: RecommendedProblem[]): 'Beginner' | 'Intermediate' | 'Advanced' {
    if (profile.totalProblemsAttempted < 10 || profile.averageAccuracy < 50) {
      return 'Beginner';
    } else if (profile.averageAccuracy >= 80 && profile.consistencyScore >= 70) {
      return 'Advanced';
    }
    return 'Intermediate';
  }
  ...
  private async analyzeRecentPerformance(userId: string, days: number): Promise<any> {
  ...
  private calculatePerformanceTrend(attempts: any[]): 'improving' | 'declining' | 'stable' {
    ...
    const earlierAvg = earlier.reduce((sum, a) => sum + a.score, 0) / earlier.length;
  ...
  private assessAdaptationNeeds(path: LearningPath, profile: UserLearningProfile, recentPerformance: any): string[] {
    const needs: string[] = [];
    
    // Check if user is consistently scoring too high or too low
    if (recentPerformance.averageScore >= 90 && recentPerformance.consistency >= 80) {
    ...
    const expectedProgress = (Date.now() - path.createdAt.getTime()) / (path.estimatedDuration * 24 * 60 * 60 * 1000) * 100;
  ...
  private async applyAdaptations(path: LearningPath, needs: string[], profile: UserLearningProfile): Promise<LearningPath> {
  ...
  private getPerformanceTrigger(need: string): string {
    const triggers = {
      'increase_difficulty': 'Consistently high scores (>90%) with high consistency',
  ...
  private findHarderProblems(profile: UserLearningProfile, count: number): RecommendedProblem[] {
    return comprehensiveProblems
      .filter(p => p.difficulty === 'Hard' && 
                   p.algorithms.some(algo => profile.strengths.includes(algo)))
      .slice(0, count)
      .map((problem, index) => this.createRecommendedProblem(problem, 999 + index, 9, 
        'Advanced challenge to match improved performance'));
  }

  private findEasierProblems(profile: UserLearningProfile, count: number): RecommendedProblem[] {
    return comprehensiveProblems
      .filter(p => p.difficulty === 'Easy' && 
                   p.algorithms.some(algo => profile.weaknesses.includes(algo)))
      .slice(0, count)
      .map((problem, index) => this.createRecommendedProblem(problem, -index, 8, 
        'Foundational reinforcement for struggling areas'));
  }

  private findReviewProblems(profile: UserLearningProfile, count: number): RecommendedProblem[] {
    return comprehensiveProblems
      .filter(p => p.difficulty === 'Easy' && 
                   p.algorithms.some(algo => Object.keys(profile.algorithmMastery).includes(algo)))
      .slice(0, count)
      .map((problem, index) => this.createRecommendedProblem(problem, -10 - index, 7, 
        'Review and reinforcement of fundamental concepts'));
  }

  private adjustProblemPacing(problems: RecommendedProblem[]): RecommendedProblem[] {
