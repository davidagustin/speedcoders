...
export interface ActivityEntry {
  id: string
  type: 'problem_solved' | 'quiz_completed' | 'achievement_unlocked' | 'streak_milestone'
  title: string
  description: string
  timestamp: string
  points?: number
...
export class ProgressTracker {
  ...
  async getUserProgress(userId: string, forceRefresh = false): Promise<UserProgress | null> {
    try {
      ...
      const [problemStats, quizStats, streakData, skillData, achievements, activities] = await Promise.all([
  ...
  async updateQuizProgress(
    ...
    try {
      ...
      for (const category of categories) {
        await this.addExperience(userId, category, Math.round(totalPoints / categories.length))
      }
  ...
  async endStudySession(
    ...
    try {
      ...
      const performance = this.calculateSessionPerformance(duration, problemsSolved, quizzesCompleted)
  ...
  async getLeaderboard(category?: string, timeFrame: 'week' | 'month' | 'all' = 'week'): Promise<Array<{
    userId: string
    username: string
    score: number
    problemsSolved: number
  ...
  private async getSkillLevels(userId: string): Promise<Record<string, SkillLevel>> {
    const { data, error } = await supabase
      .from('user_skills')
      .select('*')
      .eq('user_id', userId)
  ...
  private async addExperience(userId: string, category: string, points: number): Promise<void> {
  ...
  private async addActivity(userId: string, activity: Omit<ActivityEntry, 'id' | 'timestamp'>): Promise<void> {
    await supabase
      .from('user_activities')
      .insert({
        user_id: userId,
  ...
  private calculateSessionPerformance(duration: number, problems: number, quizzes: number): number {
