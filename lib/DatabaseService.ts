...
export class DatabaseService {
	...
	async createProblem(
		...
		try {
			...
			if (
				...
				throw new Error(
					"Missing required fields: title, difficulty, category, description",
				);
			}
	...
	async createAlgorithm(
		algorithm: Database["public"]["Tables"]["algorithms"]["Insert"],
	): Promise<Algorithm> {
		try {
			const { data, error } = await supabase
				.from("algorithms")
	...
	async createQuiz(
		quiz: Database["public"]["Tables"]["quizzes"]["Insert"],
	): Promise<Quiz> {
		try {
			const { data, error } = await supabase
				.from("quizzes")
	...
	async createQuizAttempt(
		attempt: Database["public"]["Tables"]["quiz_attempts"]["Insert"],
	): Promise<QuizAttempt> {
		try {
			const { data, error } = await supabase
				.from("quiz_attempts")
