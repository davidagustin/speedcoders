...
async function testScraper() {
	...
	try {
		...
		if (problemList.length > 0) {
			...
			const problemDetails = await scraper.getProblemDetails(
				firstProblem.titleSlug,
			);

			if (problemDetails) {
				console.log(
					`✅ Successfully fetched details for: ${problemDetails.title}`,
				);
				console.log(`   Difficulty: ${problemDetails.difficulty}`);
				console.log(
					`   Content length: ${problemDetails.content ? problemDetails.content.length : 0} characters`,
				);
