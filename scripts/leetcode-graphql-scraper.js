...
class LeetCodeGraphQLScraper {
    ...
    async saveAllProblems(problems) {
        try {
            const allProblemsFile = path.join(this.outputDir, 'all-leetcode-problems-graphql.json');
    ...
    async saveFailedProblems(failedProblems) {
        try {
            const failedFile = path.join(this.outputDir, 'failed-problems-graphql.json');
            await fs.writeFile(failedFile, JSON.stringify({
                failedAt: new Date().toISOString(),
                count: failedProblems.length,
                problems: failedProblems
            }, null, 2));
            console.log(`❌ Saved ${failedProblems.length} failed problems to ${failedFile}`);
        } catch (error) {
