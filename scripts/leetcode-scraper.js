...
class LeetCodeScraper {
    ...
    async getProblemList() {
        ...
        try {
            ...
            await this.page.waitForSelector('[data-cy="question-title"]', { timeout: 10000 });

            // Get all problem links
            const problemLinks = await this.page.evaluate(() => {
                const links = Array.from(document.querySelectorAll('[data-cy="question-title"]'));
    ...
    async scrapeProblem(problemUrl) {
        try {
            ...
            await this.page.waitForSelector('[data-track-load="description_content"]', { timeout: 10000 });
            ...
            const problemData = await this.page.evaluate(() => {
                ...
                const examplesElement = document.querySelector('[data-track-load="description_content"]');
                ...
                return {
                    ...
                    difficulty: difficultyElement ? difficultyElement.getAttribute('diff') : '',
    ...
    async scrapeAllProblems() {
        ...
        for (let i = 0; i < problemList.length; i++) {
            ...
            console.log(`Progress: ${i + 1}/${problemList.length} - ${problem.title}`);
    ...
    async saveProgress(problems) {
        try {
            ...
            await fs.writeFile(progressFile, JSON.stringify({
                ...
                problems: problems.map(p => ({ number: p.number, title: p.title, url: p.url }))
            }, null, 2));
        } catch (error) {
    ...
    async saveAllProblems(problems) {
        try {
            const allProblemsFile = path.join(this.outputDir, 'all-leetcode-problems.json');
            await fs.writeFile(allProblemsFile, JSON.stringify({
                metadata: {
            ...
            console.log(`🎉 Successfully saved ${problems.length} problems to ${allProblemsFile}`);
        } catch (error) {
...
class EnhancedLeetCodeScraper extends LeetCodeScraper {
    ...
    async scrapeWithRetry(problemUrl, retries = 0) {
        try {
            ...
            if (retries < this.maxRetries) {
                console.log(`Retrying ${problemUrl} (attempt ${retries + 1}/${this.maxRetries})`);
                await this.delay(this.rateLimitDelay * (retries + 1));
                return this.scrapeWithRetry(problemUrl, retries + 1);
            } else {
                console.error(`Failed to scrape ${problemUrl} after ${this.maxRetries} attempts`);
    ...
    async scrapeAllProblems() {
        ...
        for (let i = 0; i < problemList.length; i++) {
            ...
            if ((i + 1) % 10 === 0) {
                await this.saveProgress(results);
                console.log(`📊 Progress saved: ${i + 1}/${problemList.length} problems scraped`);
            }
        }
    ...
    async saveFailedProblems() {
        try {
            ...
            console.log(`❌ Saved ${this.failedProblems.length} failed problems to ${failedFile}`);
        } catch (error) {
