...
while ((match = problemRegex.exec(readmeContent)) !== null) {
  ...
  const difficulty = difficultyLine.includes('Easy') ? 'Easy' : 
                    difficultyLine.includes('Med.') ? 'Medium' : 'Hard';
...
function generateDescription(title, number) {
  ...
  return descriptions[title] || `Solve the problem: ${title}. This is problem number ${number} on LeetCode.`;
}
...
const solve = (root) => {
  ...
  return snippets[algorithm] || `// ${algorithm} solution
const solve = (input) => {
...
const outputPath = path.join(__dirname, '..', 'app', 'lib', 'extended-problems.ts');
