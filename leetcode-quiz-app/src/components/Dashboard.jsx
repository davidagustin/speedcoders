...
import { allLeetcodeProblems, difficultyStats, topAlgorithms } from '../data/allProblems';
...
function Dashboard({ user }) {
  ...
  const calculateUserStats = (scores) => {
    ...
    const difficultyBreakdown = scores.reduce((acc, score) => {
      acc[score.difficulty] = (acc[score.difficulty] || 0) + score.totalQuestions;
      return acc;
    ...
    for (const score of sortedScores) {
      ...
      const daysDiff = Math.floor((lastDate - scoreDate) / (1000 * 60 * 60 * 24));
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      lastWeek.push({
        ...
        avgScore: dayScores.length > 0 
          ? Math.round(dayScores.reduce((acc, s) => acc + s.percentage, 0) / dayScores.length)
          : 0
      });
    }
    return lastWeek;
    ...
    <div className="enhanced-dashboard">
      ...
      <div className="dashboard-header">
        ...
        <div className="level-progress">
          ...
          <span className="next-level">{xpForNextLevel} XP to Level {userLevel + 1}</span>
        </div>
      </div>
      ...
      <div className="quick-actions">
        ...
        <div className="action-grid">
          <div className="action-card featured" onClick={() => startQuiz('smart')}>
          ...
          <div className="action-card" onClick={() => startQuiz('company', { company: 'Google' })}>
            <div className="action-icon">🏢</div>
            <h3>Company Focus</h3>
          ...
          <div className="action-card" onClick={() => startQuiz('difficulty', { difficulty: 'Hard' })}>
            <div className="action-icon">🔥</div>
            <h3>Hard Problems</h3>
            ...
            {Object.entries(userStats.algorithmMastery)
              ...
              .map(([algorithm, stats]) => (
                <div key={algorithm} className="mastery-item">
                  ...
                  <span className="mastery-count">{stats.correct}/{stats.total}</span>
                </div>
              ))}
          </div>
          <button 
