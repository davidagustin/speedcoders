						...
						<div className="mb-4">
							...
							<div className="w-full bg-gray-700 rounded-full h-3">
								<div
									...
									style={{
										width: `${Math.min((stats.todaysGoal.completed / stats.todaysGoal.target) * 100, 100)}%`,
									}}
								></div>
							</div>
						</div>
						...
						<div className="space-y-3">
							{stats.recentActivity.slice(0, 4).map((activity, index) => (
								<div key={index} className="flex items-center justify-between">
									...
									<div
										className={`text-sm font-bold ${getScoreColor(activity.score)}`}
									>
										{activity.score}
									</div>
								</div>
