...
const dataStructures: DataStructure[] = [
  {
    ...
    operations: ['Access by index', 'Search', 'Insertion', 'Deletion', 'Traversal'],
    useCases: ['Storing sequential data', 'Matrix operations', 'Buffer management'],
    visualization: `
┌─────┬─────┬─────┬─────┬─────┐
│  1  │  2  │  3  │  4  │  5  │
└─────┴─────┴─────┴─────┴─────┘
...
// Shift elements to fill gap - O(n)`
  },
  {
    ...
    useCases: ['Dynamic memory allocation', 'Undo functionality', 'Music playlist'],
    visualization: `
┌─────┐    ┌─────┐    ┌─────┐    ┌─────┐
│  1  │───▶│  2  │───▶│  3  │───▶│  4  │
└─────┘    └─────┘    └─────┘    └─────┘
...
public:
  ...
  },
  {
    ...
    operations: ['Insertion', 'Deletion', 'Traversal (Inorder, Preorder, Postorder)', 'Search'],
...
Node* insert(Node* root, int data) {
  ...
  },
  {
    ...
    operations: ['Search', 'Insertion', 'Deletion', 'Traversal', 'Find Min/Max'],
...
Node* insert(Node* root, int data) {
  ...
  },
  {
    ...
    name: 'Hash Table',
    category: 'Hash',
    description: 'A data structure that stores key-value pairs and uses a hash function to compute an index into an array of buckets.',
    complexity: {
      time: {
        access: 'O(1)',
        search: 'O(1)',
...
export default function DataStructuresPage() {
  const [structures, setStructures] = useState<DataStructure[]>(dataStructures)
  ...
  const filteredStructures = structures.filter(structure => {
    const categoryMatch = selectedCategory === 'all' || structure.category === selectedCategory
    return categoryMatch
  })

  const categories = [...new Set(structures.map(structure => structure.category))]
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="px-4 py-6 sm:px-0">
          <h1 className="text-3xl font-bold text-gray-900">Data Structures Visualizer</h1>
          <p className="mt-2 text-gray-600">Learn and visualize different data structures and their operations</p>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white shadow rounded-lg">
              <div className="px-4 py-5 sm:p-6">
                <h3 className="text-lg font-medium text-gray-900 mb-4">Data Structures</h3>
                  ...
                  {filteredStructures.map((structure) => (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        ...
                        <span className="text-xs text-gray-500">Access: {structure.complexity.time.access}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {selectedStructure ? (
              <div className="space-y-6">
                ...
                <div className="bg-white shadow rounded-lg p-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h2 className="text-xl font-semibold text-gray-900">{selectedStructure.name}</h2>
                ...
                <div className="bg-white shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-3">Visualization</h3>
                    <div className={`bg-gray-900 text-green-400 p-4 rounded-lg font-mono text-sm transition-all duration-${animationSpeed} ${
                ...
                <div className="bg-white shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    <h3 className="text-lg font-medium text-gray-900 mb-3">Time & Space Complexity</h3>
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm">
                          <div className="flex justify-between">
                            <span>Access:</span>
                            <span className="font-mono">{selectedStructure.complexity.time.access}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Search:</span>
                            <span className="font-mono">{selectedStructure.complexity.time.search}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Insertion:</span>
                            <span className="font-mono">{selectedStructure.complexity.time.insertion}</span>
                          </div>
                          <div className="flex justify-between">
                            <span>Deletion:</span>
                            <span className="font-mono">{selectedStructure.complexity.time.deletion}</span>
                          </div>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="text-sm">
                          <span className="font-mono">{selectedStructure.complexity.space}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white shadow rounded-lg">
                    <div className="px-4 py-5 sm:p-6">
                      <h3 className="text-lg font-medium text-gray-900 mb-3">Common Operations</h3>
              ...
              <div className="bg-white shadow rounded-lg">
                <div className="px-4 py-5 sm:p-6 text-center">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900">Select a Data Structure</h3>
                  <p className="mt-1 text-sm text-gray-500">Choose a data structure from the list to learn more about it.</p>
                </div>
              </div>
            )}
          </div>
