...
function generateProblemsRange(startId, endId, baseAlgorithms) {
  ...
  for (let id = startId; id <= endId; id++) {
    ...
    const titlePatterns = [
      "Maximum {concept}", "Minimum {concept}", "Find {concept}", "Count {concept}",
      "Valid {concept}", "Remove {concept}", "Insert {concept}", "Delete {concept}",
      "Search {concept}", "Sort {concept}", "Merge {concept}", "Split {concept}",
      "Reverse {concept}", "Rotate {concept}", "Clone {concept}", "Design {concept}",
      "Implement {concept}", "Check {concept}", "Verify {concept}", "Build {concept}"
    ];
    
    const concepts = [
      "Array", "Tree", "Graph", "String", "Number", "List", "Stack", "Queue",
      "Matrix", "Binary Tree", "Linked List", "Substring", "Subarray", "Path",
      "Distance", "Sum", "Product", "Element", "Node", "Value", "Key", "Pair"
    ];
    ...
    if (title.includes("Tree") || title.includes("Binary")) {
      ...
      algorithms = ["Graph", "DFS", "BFS", "Union Find", "Topological Sort", "Dijkstra"];
      correctAlgorithms = ["Graph", "DFS", "BFS"];
    } else if (title.includes("List") || title.includes("Linked")) {
      algorithms = ["Linked List", "Two Pointers", "Recursion", "Stack", "Fast & Slow"];
      correctAlgorithms = ["Linked List", "Two Pointers"];
    } else if (title.includes("Stack") || title.includes("Queue")) {
      algorithms = ["Stack", "Queue", "Design", "Simulation", "Monotonic Stack"];
      correctAlgorithms = ["Stack", "Design"];
    } else if (title.includes("Search") || title.includes("Find")) {
      algorithms = ["Binary Search", "Hash Table", "Two Pointers", "DFS", "BFS"];
      correctAlgorithms = ["Binary Search", "Hash Table"];
    } else if (title.includes("Sort")) {
      algorithms = ["Sorting", "Merge Sort", "Quick Sort", "Heap", "Bucket Sort"];
      correctAlgorithms = ["Sorting", "Merge Sort"];
    } else if (title.includes("Dynamic") || title.includes("Maximum") || title.includes("Minimum")) {
      algorithms = ["Dynamic Programming", "Greedy", "Math", "Optimization", "Memoization"];
      correctAlgorithms = ["Dynamic Programming", "Greedy"];
    } else {
      // Default algorithm set
      algorithms = baseAlgorithms || ["Array", "Hash Table", "Math", "Two Pointers", "Greedy"];
    ...
    problems.push({
      ...
      title,
      difficulty,
      url: `https://leetcode.com/problems/${title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')}/`,
...
export const batch3Problems = generateProblemsRange(1001, 1500, ["Dynamic Programming", "Graph", "Tree"]);
export const batch4Problems = generateProblemsRange(1501, 2000, ["String", "Array", "Hash Table"]);
export const batch5Problems = generateProblemsRange(2001, 2500, ["Math", "Greedy", "Sorting"]);
export const batch6Problems = generateProblemsRange(2501, 3000, ["Binary Search", "Two Pointers", "Sliding Window"]);
export const batch7Problems = generateProblemsRange(3001, 3662, ["Backtracking", "DFS", "BFS"]);