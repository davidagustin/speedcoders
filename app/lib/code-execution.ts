...
export class CodeExecutor {
  ...
  private memoryLimit: number = 128; // 128MB
    ...
    try {
      ...
      for (const testCase of request.testCases) {
        ...
        try {
          ...
          const result = await this.executeWithTimeout(safeCode, parsedInput, this.timeLimit);
          const actualOutput = typeof result === 'object' ? JSON.stringify(result) : String(result);
      ...
      return {
        success: allPassed,
        output: testResults.map(r => `${r.input} → ${r.actualOutput}`).join('\n'),
        executionTime: Date.now() - startTime,
  ...
  private async executePython(request: CodeExecutionRequest): Promise<CodeExecutionResult> {
    ...
    return {
      success: false,
      output: '',
      error: 'Python execution not implemented - would require server-side runtime',
      executionTime: 0
    };
  }

  private async executeJava(request: CodeExecutionRequest): Promise<CodeExecutionResult> {
    // For now, return a mock result as we'd need a Java runtime
    return {
      success: false,
      output: '',
      error: 'Java execution not implemented - would require server-side runtime',
      executionTime: 0
    };
  }

  private async executeCpp(request: CodeExecutionRequest): Promise<CodeExecutionResult> {
    // For now, return a mock result as we'd need a C++ compiler
    return {
      success: false,
      output: '',
      error: 'C++ execution not implemented - would require server-side compiler',
      executionTime: 0
    };
  }
  ...
  private estimateSpaceComplexity(code: string): string {
    ...
    if (code.includes('new Array') || code.includes('[]')) {
      if (code.includes('Array(n)') || code.includes('new Array(n)')) return 'O(n)';
      return 'O(n)';
    }
    if (code.includes('new Map') || code.includes('new Set')) return 'O(n)';
    if (code.includes('recursion') || code.includes('function.*function')) return 'O(n)';
  ...
  private calculateCyclomaticComplexity(code: string): number {
    ...
    complexity += ifStatements + elseStatements + switchCases + loops + ternary + catches + logicalOps;
