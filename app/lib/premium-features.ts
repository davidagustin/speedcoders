...
export class PremiumService {
	...
	private initializePremiumFeatures() {
		this.premiumFeatures = [
			// Analytics Features
			{
				...
				description:
					"Detailed performance insights with trends and predictions",
				category: "analytics",
				tier: "premium",
				isEnabled: false,
			},
	...
	checkUsageLimits(
		...
		switch (feature) {
			case "problems":
				return {
					allowed:
						!limits.problemsPerMonth ||
						usage.problemsSolved < limits.problemsPerMonth,
			...
			case "analytics":
				return {
					allowed:
						!limits.analyticsHistory ||
						usage.analyticsAccessed < limits.analyticsHistory,
			...
			case "customQuizzes":
				return {
					allowed:
						!limits.customQuizzes || usage.customQuizzes < limits.customQuizzes,
			...
			case "aiRecommendations":
				return {
					allowed:
						!limits.aiRecommendations ||
						usage.aiRecommendations < limits.aiRecommendations,
			...
			case "codeExecution":
				return {
					allowed:
						!limits.codeExecution ||
						usage.codeExecutions < limits.codeExecution,
	...
	compareFeatures(
		...
		newFeatures: string[];
		upgradedLimits: {
			[key: string]: { current: number | boolean; new: number | boolean };
		};
	} {
