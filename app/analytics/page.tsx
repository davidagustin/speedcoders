	...
	const fetchAnalytics = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/analytics?userId=demo&period=${period}`,
			);
						...
						<div className="mt-4">
							<div className="flex justify-between text-sm">
								...
								<span className="text-green-600">
									{analytics.overview.completedQuizzes}
								</span>
							</div>
							<div className="w-full bg-gray-200 rounded-full h-2 mt-1">
								<div
									...
									style={{
										width: `${(analytics.overview.completedQuizzes / analytics.overview.totalQuizzes) * 100}%`,
									}}
								></div>
							</div>
						</div>
					...
					<div className="bg-white p-6 rounded-lg shadow">
						...
						<div className="mt-4">
							<div className="flex justify-between text-sm">
								...
								<span
									className={
										analytics.overview.averageScore >= 80
											? "text-green-600"
											: analytics.overview.averageScore >= 60
												? "text-yellow-600"
												: "text-red-600"
									}
								>
						...
						<div className="mt-4">
							<div className="flex justify-between text-sm">
								...
								<span className="text-purple-600">
									{analytics.overview.problemsSolved}
								</span>
							</div>
						</div>
					</div>
				...
				<div className="mt-8 bg-white p-6 rounded-lg shadow">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div>
							...
							<div className="space-y-3">
								{analytics.recentActivity.recentAchievements.map(
									(achievement, index) => (
									...
									<div>
										...
										<p className="text-xs text-gray-600">
											{analytics.recentActivity.lastQuiz
												? formatDate(analytics.recentActivity.lastQuiz)
												: "No quizzes yet"}
										</p>
									</div>
								</div>
									...
									<div>
										...
										<p className="text-xs text-gray-600">
											{analytics.recentActivity.lastStudySession
												? formatDate(analytics.recentActivity.lastStudySession)
												: "No sessions yet"}
										</p>
									</div>
								</div>
