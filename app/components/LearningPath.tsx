...
export default function LearningPath() {
	...
	const generateLearningPath = async () => {
        ...
        `)
			...
			const completedProblems = new Set<string>(
				attempts
					? attempts.map((attempt: any) => attempt.problemTitle).filter(Boolean)
					: [],
			);
			...
			const recommendedProblems = getRecommendedProblems(
				userLevel,
				selectedCategory,
				completedProblems as Set<string>,
			);
	...
	const getRecommendedProblems = (
		...
		const recommendedProblems = problemsWithMetadata
			.slice(0, 10)
			.map((problem, index) => ({
				...problem,
				...
				{learningPath.length === 0 ? (
					<div className="text-center py-8">
						...
						<p className="text-gray-600">
							Great job! You've completed all problems in this category.
						</p>
						<p className="text-sm text-gray-500 mt-1">
							Try a different category or increase the difficulty.
						</p>
					</div>
				) : (
			...
			{learningPath.length > 0 && (
				<div className="mt-6 pt-4 border-t border-gray-200">
					<div className="flex items-center justify-between text-sm text-gray-600">
						...
						<span>
							{learningPath.filter((p) => p.isCompleted).length} /{" "}
							{learningPath.length} completed
						</span>
					</div>
