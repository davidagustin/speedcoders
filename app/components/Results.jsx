...
function Results({ user, quizResult }) {
	...
	const [_selectedQuestion, _setSelectedQuestion] = useState(null);
	...
	const getPerformanceLevel = (percentage) => {
		...
		if (percentage >= 80)
			return { level: "Good", color: "#2196F3", icon: "👏" };
		if (percentage >= 70)
			return { level: "Fair", color: "#FF9800", icon: "👍" };
		if (percentage >= 60)
			return { level: "Needs Work", color: "#FF5722", icon: "📚" };
	...
	const getTimePerformance = () => {
		...
		if (timeUsedPercentage <= 50)
			return { rating: "Fast", color: "#4CAF50", icon: "⚡" };
		if (timeUsedPercentage <= 75)
			return { rating: "Good Pace", color: "#2196F3", icon: "⏱️" };
		if (timeUsedPercentage <= 90)
			return { rating: "Steady", color: "#FF9800", icon: "🐢" };
		...
		return (
			<div className={`question-detail ${isCorrect ? "correct" : "incorrect"}`}>
				...
				<div className="question-info">
					<span
						className={`difficulty-badge ${detail.difficulty.toLowerCase()}`}
					>
						{detail.difficulty}
					</span>
					<a
								...
								detail.userAnswer.map((algo) => (
									<span
										...
										className={`algorithm-tag ${
											detail.correctAnswer.includes(algo)
												? "correct"
												: "incorrect"
										}`}
									>
				...
				{!isCorrect && (
					<div className="explanation">
						...
						<p>
							The correct algorithms for this problem are optimal because they
							provide the best time/space complexity trade-offs for the given
							constraints.
						</p>
					</div>
				)}
				...
				<div className="breakdown-section">
					...
					<div className="stats-grid">
						<div className="stat-card">
							<div
								className="stat-icon"
								style={{ color: timePerformance.color }}
							>
