...
import { achievementSystem, Achievement, Badge } from '../lib/achievements';
...
export default function AchievementCenter() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'solving' | 'streak' | 'speed' | 'accuracy' | 'algorithm' | 'special'>('all');
  ...
  const loadAchievements = () => {
    ...
    const secretAchievements = showSecrets ? achievementSystem.getSecretAchievements() : [];
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl p-8 text-white mb-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="text-center">
            ...
            <div className="w-full bg-blue-700 rounded-full h-3 mt-2">
              <div
                ...
                style={{ width: `${(userStats.currentExp / userStats.nextLevelExp) * 100}%` }}
              />
            </div>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold">{userStats.earnedAchievements}</div>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold">{userStats.totalExperience.toLocaleString()}</div>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className={`inline-block px-2 py-1 text-xs font-semibold rounded-full ${getRarityColor(achievement.rarity)}`}>
                      {achievement.rarity}
                    </span>
                  </div>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-600">
                  {achievement.isSecret && !isEarned ? 'Hidden achievement' : achievement.description}
                </p>
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {userStats.badges.map((badge) => (
              <div key={badge.id} className="text-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                ...
                <div className={`text-xs px-2 py-1 rounded-full mt-2 ${getRarityColor(badge.rarity)}`}>
                  {badge.rarity}
                </div>
              </div>
            ))}
