...
import { allLeetcodeProblems, companyTags, algorithmCategories } from '../data/allProblems';
...
function Quiz({ user, onComplete }) {
  ...
  const generateQuiz = () => {
    ...
    switch (config.mode) {
      ...
      case 'algorithm':
        if (config.algorithms.length > 0) {
          selectedProblems = selectedProblems.filter(p => 
            p.correctAlgorithms.some(algo => config.algorithms.includes(algo))
          );
      ...
      case 'speed':
        ...
        selectedProblems = selectedProblems.filter(p => p.difficulty === 'Easy');
        break;
    ...
    if (config.difficulty !== 'Mixed' && config.mode !== 'difficulty') {
      selectedProblems = selectedProblems.filter(p => p.difficulty === config.difficulty);
    }
  ...
  const getBalancedMix = () => {
    const easy = allLeetcodeProblems.filter(p => p.difficulty === 'Easy').slice(0, 1000);
    const medium = allLeetcodeProblems.filter(p => p.difficulty === 'Medium').slice(0, 1000);
    const hard = allLeetcodeProblems.filter(p => p.difficulty === 'Hard').slice(0, 500);
            ...
            <div className="info-card">
              ...
              <div className="quiz-stats">
                <div className="stat">
                  ...
                  <span className="value">{quizConfig.mode.charAt(0).toUpperCase() + quizConfig.mode.slice(1)}</span>
                </div>
                <div className="stat">
                ...
                <div className="stat">
                  ...
                  <span className="value">{formatTime(quizConfig.timeLimit)}</span>
                </div>
                <div className="stat">
            ...
            {quizOptions.challenge && (
              <div className="challenge-info">
                ...
                <div className="challenge-details">
                  <span className="challenge-icon">{quizOptions.challenge.icon}</span>
                  <div>
                    <h4>{quizOptions.challenge.title}</h4>
                    <p>{quizOptions.challenge.description}</p>
                    <span className="challenge-reward">Reward: {quizOptions.challenge.reward}</span>
                  </div>
                </div>
              </div>
            )}
  ...
  if (questions.length === 0) return <div className="loading">Loading quiz...</div>;
  ...
  return (
    <div className="enhanced-quiz">
      ...
      <div className="quiz-stats-sidebar">
        ...
        <div className="stat">
          <span>Answered:</span>
          <span>{Object.values(answers).filter(ans => ans.length > 0).length}/{questions.length}</span>
        </div>
        <div className="stat">
          <span>Mode:</span>
