...
function ProblemBrowser({ user }) {
	...
	// Use search results if available, otherwise use all problems
	const baseProblems = searchResults ? searchResults : allLeetcodeProblems;

	// Filter and sort problems
	const filteredProblems = useMemo(() => {
		const filtered = baseProblems.filter((problem) => {
			// Search filter
			const matchesSearch =
			...
			const matchesCompany =
				...
				(companyTags[filters.company] &&
					companyTags[filters.company].includes(problem.id));
		...
		return filtered;
	}, [baseProblems, searchTerm, filters, sortBy, solvedProblems]);
	...
	const createCustomQuiz = () => {
		...
		const _selectedProblemsList = Array.from(selectedProblems);
		const selectedProblemsData = (searchResults || allLeetcodeProblems).filter(
			(p) => selectedProblems.has(p.id),
		);
	...
	const handleAdvancedSearchResults = (results, _searchInfo) => {
			...
			<div className={`problem-card ${viewMode} ${isSolved ? "solved" : ""}`}>
				...
				<div className="problem-algorithms">
					{problem.correctAlgorithms
						.slice(0, viewMode === "detailed" ? 10 : 3)
						.map((algo) => (
							<span key={algo} className="algorithm-tag">
								{algo}
							</span>
						))}
					{problem.correctAlgorithms.length >
						(viewMode === "detailed" ? 10 : 3) && (
		...
		<div className="problem-browser">
				...
				<div className="search-section">
					...
					{searchResults && (
						<div className="search-results-info">
							📊 Showing {filteredProblems.length} results from advanced search
						</div>
					)}
				</div>
			...
			<div className="results-info">
				<span>
					...
					{Math.min(startIndex + problemsPerPage, filteredProblems.length)}
