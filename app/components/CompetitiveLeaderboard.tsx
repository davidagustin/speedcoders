...
export default function CompetitiveLeaderboard() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'global' | 'friends' | 'country'>('global');
  ...
  const fetchLeaderboardData = async () => {
    ...
    const mockData: LeaderboardEntry[] = Array.from({ length: 100 }, (_, index) => ({
      id: `user-${index + 1}`,
  ...
  const generateUsername = (): string => {
    const adjectives = ['Swift', 'Clever', 'Brilliant', 'Sharp', 'Quick', 'Smart', 'Wise', 'Nimble', 'Fast', 'Keen'];
    const nouns = ['Coder', 'Dev', 'Algo', 'Binary', 'Logic', 'Code', 'Script', 'Debug', 'Loop', 'Function'];
  ...
  const generateBadges = (): Badge[] => {
    const allBadges = [
      ...
      { id: 'perfectionist', name: 'Perfectionist', icon: '💯', rarity: 'epic' as const },
      { id: 'streak-master', name: 'Streak Master', icon: '🔥', rarity: 'rare' as const },
      { id: 'problem-solver', name: 'Problem Solver', icon: '🧩', rarity: 'common' as const },
      { id: 'algorithm-expert', name: 'Algorithm Expert', icon: '🎯', rarity: 'legendary' as const }
    ];
    
    return allBadges.slice(0, Math.floor(Math.random() * 4) + 1).map(badge => ({
      ...badge,
  ...
  const getRankIcon = (rank: number) => {
    if (rank === 1) return <TrophySolidIcon className="h-6 w-6 text-yellow-500" />;
    if (rank === 2) return <TrophySolidIcon className="h-6 w-6 text-gray-400" />;
    if (rank === 3) return <TrophySolidIcon className="h-6 w-6 text-amber-600" />;
    return null;
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Competitive Leaderboard</h1>
          <p className="text-gray-600 mt-1">Compete with coders worldwide and climb the ranks</p>
        </div>
            ...
            <div className="text-right">
              ...
              <div className="w-32 bg-blue-700 rounded-full h-3 mt-2">
                <div
                  ...
                  style={{ width: `${(currentUser.experience / currentUser.nextLevelExp) * 100}%` }}
                />
              </div>
          ...
          {tournaments.map((tournament) => (
            <div key={tournament.id} className={`border rounded-lg p-4 ${
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <div>👥 {tournament.participants.toLocaleString()} participants</div>
                <div>🎯 {tournament.difficulty} difficulty</div>
                <div className="font-medium text-gray-900">{tournament.prize}</div>
              </div>
              
              <button className={`mt-3 w-full px-4 py-2 text-sm font-medium rounded-lg transition-colors ${
                tournament.status === 'active'
                ...
                {tournament.status === 'active' ? 'Join Now' : 
                 tournament.status === 'upcoming' ? 'Register' : 'Completed'}
              </button>
            </div>
          ))}
        </div>
