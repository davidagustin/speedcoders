...
function Analytics({ user }) {
	...
	const calculateTotalStats = (scores) => {
		...
		const avgAccuracy =
			totalProblems > 0 ? (totalCorrect / totalProblems) * 100 : 0;
		const totalTime = scores.reduce(
			(sum, score) => sum + (score.timeSpent || 0),
			0,
		);
		const avgTimePerProblem = totalProblems > 0 ? totalTime / totalProblems : 0;
		const totalXP = scores.reduce(
			(sum, score) => sum + (score.xpEarned || 0),
			0,
		);
		...
		Object.keys(mastery).forEach((algorithm) => {
			...
			data.recentAccuracy =
				recentAttempts.length > 0
					...
					: 0;
			data.trend =
				data.recentAccuracy > data.accuracy
					? "improving"
					: data.recentAccuracy < data.accuracy
						? "declining"
						: "stable";
	...
	const calculateTimeAnalysis = (scores) => {
		...
		return {
			avgTimeByDifficulty,
			timeByAccuracy,
			fastestSolve: Math.min(
				...scores
					.map((s) => s.timeSpent / s.totalQuestions)
					.filter((t) => t > 0),
			),
			slowestSolve: Math.max(
				...scores.map((s) => s.timeSpent / s.totalQuestions),
			),
		};
	};
		...
		sortedScores.forEach((score) => {
			...
			if (lastDate) {
				const daysDiff = Math.floor(
					(scoreDate - lastDate) / (1000 * 60 * 60 * 24),
				);
				if (daysDiff === 1) {
					currentStreak++;
				} else if (daysDiff > 1) {
	...
	const generateRecommendations = (weaknesses, _strengths, _mastery) => {
		...
		if (analytics.timeAnalysis.avgTimeByDifficulty?.Easy > 120) {
			recommendations.push({
				...
				description:
					"You're taking too long on Easy problems. Practice for speed.",
		...
		<div className="analytics-dashboard">
			...
			<div className="key-metrics">
				<div className="metric-card">
					...
					<div className="metric-content">
						...
						<div className="metric-value">
							{analytics.totalStats.avgAccuracy}%
						</div>
						<div className="metric-detail">
							{analytics.totalStats.totalCorrect}/
							{analytics.totalStats.totalProblems} correct
						</div>
					</div>
				</div>
				...
				<div className="metric-card">
					...
					<div className="metric-content">
						...
						<div className="metric-value">
							{analytics.totalStats.avgTimePerProblem}s
						</div>
						<div className="metric-detail">
			...
			{analytics.performanceTrends.length > 0 && (
				<div className="performance-trends">
					...
					<div className="trend-chart">
						{analytics.performanceTrends.map((trend, _index) => (
			...
			<div className="difficulty-analysis">
				...
				<div className="difficulty-grid">
					{Object.entries(analytics.difficultyProgress).map(
						([difficulty, data]) => (
							<div
								key={difficulty}
								className={`difficulty-card ${difficulty.toLowerCase()}`}
							>
								<h3>{difficulty}</h3>
				...
				<div className="time-grid">
					<div className="time-card">
						...
						<div className="time-breakdown">
							{Object.entries(
								analytics.timeAnalysis.avgTimeByDifficulty || {},
							).map(([diff, time]) => (
								<div key={diff} className="time-item">
									<span className={`diff-label ${diff.toLowerCase()}`}>
										{diff}
									</span>
