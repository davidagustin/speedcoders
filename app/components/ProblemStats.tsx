        ...
        `)
			...
			if (attempts) {
				...
				const recentActivity = attempts.slice(0, 5).map((attempt) => ({
					problemTitle:
						attempt.quiz?.questions?.[0]?.problem?.title || "Unknown",
					score: attempt.score,
					date: new Date(
		...
		return (
			<div className="bg-white rounded-lg shadow-md p-6">
				...
				<p className="text-gray-600">
					No data available. Start solving problems to see your statistics!
				</p>
			</div>
		);
	}
				...
				<div className="text-center">
					<div className="text-2xl font-bold text-orange-600">
						{stats.totalProblems > 0
							? Math.round(
									(stats.completedProblems / stats.totalProblems) * 100,
								)
							: 0}
						%
					</div>
						...
						<div className="flex items-center space-x-2">
							<div className="w-24 bg-gray-200 rounded-full h-2">
								<div
									...
									style={{
										width: `${stats.totalProblems > 0 ? (stats.easyProblems / stats.totalProblems) * 100 : 0}%`,
									}}
								></div>
							</div>
						...
						<div className="flex items-center space-x-2">
							<div className="w-24 bg-gray-200 rounded-full h-2">
								<div
									...
									style={{
										width: `${stats.totalProblems > 0 ? (stats.mediumProblems / stats.totalProblems) * 100 : 0}%`,
									}}
								></div>
							</div>
							<span className="text-sm font-medium text-gray-900">
						...
						<div className="flex items-center space-x-2">
							<div className="w-24 bg-gray-200 rounded-full h-2">
								<div
									...
									style={{
										width: `${stats.totalProblems > 0 ? (stats.hardProblems / stats.totalProblems) * 100 : 0}%`,
									}}
								></div>
							</div>
							<span className="text-sm font-medium text-gray-900">
