...
function AlgorithmMastery({ user }) {
	...
	const getMasteryLevel = (stats) => {
		if (!stats || stats.total === 0)
			return { level: "Beginner", color: "#9E9E9E", progress: 0 };

		const accuracy = stats.accuracy;
		const problemCount = stats.total;

		if (accuracy >= 90 && problemCount >= 20)
			return { level: "Master", color: "#4CAF50", progress: 100 };
		if (accuracy >= 80 && problemCount >= 15)
			return { level: "Advanced", color: "#2196F3", progress: 80 };
		if (accuracy >= 70 && problemCount >= 10)
			return { level: "Intermediate", color: "#FF9800", progress: 60 };
		if (accuracy >= 60 && problemCount >= 5)
			return { level: "Novice", color: "#9C27B0", progress: 40 };
	...
	const filteredAlgorithms = useMemo(() => {
		...
		if (selectedCategory !== "all") {
			...
			algorithms = algorithms.filter(([algo]) =>
				categoryAlgorithms.includes(algo),
			);
		}
		...
		algorithms.sort((a, b) => {
			...
			switch (sortBy) {
				case "mastery":
					return (
						getMasteryLevel(statsB).progress - getMasteryLevel(statsA).progress
					);
				case "accuracy":
		...
		return algorithms;
	}, [algorithmMastery, selectedCategory, sortBy, getMasteryLevel]);
		...
		return (
			<div className={`algorithm-card ${viewMode}`}>
				...
				<div className="algorithm-actions">
					...
					{stats.total === 0 && (
						<button className="learn-btn">Learn Algorithm</button>
					)}
				</div>
			</div>
		);
		...
		<div className="algorithm-mastery">
			...
			<div className="mastery-overview">
				...
				<div className="mastery-distribution">
					...
					<div className="mastery-chart">
						{Object.entries(overallStats.masteryLevels).map(
							([level, count]) => {
								const colors = {
									Master: "#4CAF50",
									Advanced: "#2196F3",
			...
			<div className={`algorithms-container ${viewMode}`}>
				{filteredAlgorithms.map(([algorithm, stats]) => (
					<AlgorithmCard key={algorithm} algorithm={algorithm} stats={stats} />
				))}
			</div>
					...
					{Object.entries(algorithmMastery)
						...
						.map(([algo, stats]) => (
							<div key={algo} className="recommendation-card weakness">
								...
								<button className="recommendation-action">Practice More</button>
							</div>
						))}
