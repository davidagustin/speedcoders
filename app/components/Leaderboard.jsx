...
function Leaderboard({ user }) {
	...
	const [globalLeaderboard, _setGlobalLeaderboard] = useState([
		{
			id: "global-1",
			name: "CodeMaster2024",
			xp: 15750,
			...
			.map((u) => {
				...
				if (u.scores) {
					...
					if (u.scores.length > 0) {
						const sortedScores = u.scores.sort(
	...
	const filteredLeaderboard = useMemo(() => {
		...
		return localLeaderboard.filter(
			(u) => u.lastActivity && new Date(u.lastActivity) >= cutoffDate,
		);
	}, [localLeaderboard, timeframe]);

	// Get user's rank
	const currentUserRank =
		filteredLeaderboard.findIndex((u) => u.id === user.id) + 1;
	...
	const getAchievementBadges = (userData) => {
		...
		if (userData.problemsSolved >= 1000)
			badges.push({ icon: "🏆", title: "Problem Master", color: "#FFD700" });
		else if (userData.problemsSolved >= 500)
			badges.push({ icon: "🥇", title: "Problem Crusher", color: "#FF6B35" });
		else if (userData.problemsSolved >= 100)
			badges.push({ icon: "🥈", title: "Problem Solver", color: "#C0C0C0" });

		if (userData.accuracy >= 95)
			badges.push({ icon: "🎯", title: "Perfectionist", color: "#4CAF50" });
		else if (userData.accuracy >= 85)
			badges.push({ icon: "🏹", title: "Sharpshooter", color: "#2196F3" });

		if (userData.streak >= 50)
			badges.push({ icon: "🔥", title: "Streak Legend", color: "#FF5722" });
		else if (userData.streak >= 20)
			badges.push({ icon: "⚡", title: "Streak Master", color: "#FF9800" });
		else if (userData.streak >= 7)
			badges.push({ icon: "💪", title: "Dedicated", color: "#9C27B0" });

		if (userData.level >= 20)
			badges.push({ icon: "👑", title: "Coding Royalty", color: "#FFD700" });
		else if (userData.level >= 10)
			badges.push({ icon: "🌟", title: "Rising Star", color: "#FFC107" });
		...
		<div className="leaderboard">
			...
			<div className="leaderboard-list">
				<div className="list-header">
					<h2>
						{showGlobal ? "🌍 Global" : "📍 Local"} Leaders -{" "}
						{category.charAt(0).toUpperCase() + category.slice(1)}
					</h2>
					<span className="participant-count">
						{showGlobal ? globalLeaderboard.length : filteredLeaderboard.length}{" "}
						participants
					</span>
				</div>

				<div className="leaderboard-rows">
					{(showGlobal ? globalLeaderboard : filteredLeaderboard).map(
						(userData, index) => (
							<LeaderboardRow
								key={userData.id}
								userData={userData}
					...
					<div className="competition-stats">
						...
						<div className="stat">
							...
							<span className="value">
								{currentUser?.problemsSolved || 0} problems
							</span>
						</div>
						<div className="stat">
					...
					<div className="competition-rewards">
						...
						<div className="rewards-list">
							...
							<span className="reward">
								🥈 2nd-10th: 2000 XP + Silver Badge
							</span>
							<span className="reward">
								🥉 11th-100th: 1000 XP + Bronze Badge
							</span>
						</div>
					</div>
				</div>
