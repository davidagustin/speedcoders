...
const CATEGORIES = [
  ...
  'Two Pointers', 'Stack', 'Heap', 'Graph', 'Design', 'Simulation', 'Backtracking',
  'Bit Manipulation', 'Union Find', 'Linked List', 'Sliding Window', 'Trie'
];

const ALGORITHMS = [
  'Two Pointers', 'Sliding Window', 'Binary Search', 'DFS', 'BFS', 'Dynamic Programming',
  'Greedy', 'Divide and Conquer', 'Backtracking', 'Union Find', 'Topological Sort',
  'Dijkstra', 'A*', 'KMP', 'Rabin-Karp', 'Manacher', 'Morris Traversal'
];

const COMPANIES = [
  'Google', 'Facebook', 'Amazon', 'Microsoft', 'Apple', 'Netflix', 'Uber', 'Airbnb',
  'LinkedIn', 'Twitter', 'Spotify', 'Dropbox', 'Salesforce', 'Adobe', 'Oracle'
];

const TIME_COMPLEXITIES = ['O(1)', 'O(log n)', 'O(n)', 'O(n log n)', 'O(n²)', 'O(n³)', 'O(2ⁿ)'];
...
export default function AdvancedSearch() {
  ...
  const [sortBy, setSortBy] = useState<'relevance' | 'difficulty' | 'acceptance' | 'frequency' | 'title'>('relevance');
  ...
  const generateProblemTitle = (): string => {
    const actions = ['Find', 'Maximum', 'Minimum', 'Count', 'Sum', 'Search', 'Sort', 'Reverse'];
    const objects = ['Array', 'Tree', 'Path', 'Substring', 'Number', 'Elements', 'Nodes', 'String'];
    const descriptors = ['Longest', 'Shortest', 'Valid', 'Unique', 'Common', 'Missing', 'Duplicate'];
  ...
  const filteredProblems = useMemo(() => {
    let filtered = problems.filter(problem => {
      // Text search
      if (filters.query && !problem.title.toLowerCase().includes(filters.query.toLowerCase())) {
        return false;
      }
      ...
      // Algorithm filter
      if (filters.algorithms.length > 0 && !filters.algorithms.some(alg => problem.algorithms.includes(alg))) {
        return false;
      }

      // Company filter
      if (filters.companies.length > 0 && !filters.companies.some(comp => problem.companies.includes(comp))) {
        return false;
      }
      ...
      if (problem.acceptance < filters.acceptance[0] || problem.acceptance > filters.acceptance[1]) {
        return false;
      }

      // Frequency filter
      if (problem.frequency < filters.frequency[0] || problem.frequency > filters.frequency[1]) {
        return false;
      }

      // Estimated time filter
      if (problem.estimatedTime < filters.estimatedTime[0] || problem.estimatedTime > filters.estimatedTime[1]) {
        return false;
      }
  ...
  const toggleArrayFilter = <K extends keyof Pick<SearchFilters, 'difficulty' | 'categories' | 'algorithms' | 'companies'>>(
    key: K,
    value: string
  ) => {
    setFilters(prev => ({
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Advanced Problem Search</h1>
        <p className="text-gray-600">Find the perfect problems to practice with powerful filtering and search</p>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Acceptance Rate: {filters.acceptance[0]}% - {filters.acceptance[1]}%
              </label>
              <input
                type="range"
                min="0"
                ...
                onChange={(e) => updateFilter('acceptance', [filters.acceptance[0], parseInt(e.target.value)])}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateFilter('frequency', [filters.frequency[0], parseInt(e.target.value)])}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              />
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Est. Time: {filters.estimatedTime[0]} - {filters.estimatedTime[1]} min
              </label>
              <input
                type="range"
                min="0"
                ...
                onChange={(e) => updateFilter('estimatedTime', [filters.estimatedTime[0], parseInt(e.target.value)])}
                className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
              />
            </div>
          </div>
      ...
      <div className="space-y-4">
        {filteredProblems.map((problem) => (
          <div key={problem.id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                ...
                <p className="text-gray-700 text-sm mb-3 line-clamp-2">{problem.description}</p>
