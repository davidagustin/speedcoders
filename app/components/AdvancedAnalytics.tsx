...
export default function AdvancedAnalytics() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'categories' | 'algorithms' | 'competitive'>('overview');
  ...
  const fetchAnalyticsData = async () => {
    ...
    const mockData: AnalyticsData = {
      ...
      rank: 1543,
      categoryBreakdown: [
        { category: 'Array', solved: 45, total: 120, accuracy: 82.3, averageTime: 18.2 },
        { category: 'Dynamic Programming', solved: 23, total: 85, accuracy: 65.4, averageTime: 35.7 },
        { category: 'Tree', solved: 34, total: 95, accuracy: 88.2, averageTime: 21.3 },
        { category: 'Graph', solved: 18, total: 76, accuracy: 72.1, averageTime: 42.1 },
        { category: 'String', solved: 39, total: 102, accuracy: 79.5, averageTime: 19.8 },
        { category: 'Hash Table', solved: 28, total: 78, accuracy: 84.6, averageTime: 16.4 }
      ],
      difficultyBreakdown: [
        { difficulty: 'Easy', solved: 156, total: 234, accuracy: 89.2, averageTime: 12.3, trending: 'stable' },
        { difficulty: 'Medium', solved: 142, total: 456, accuracy: 74.6, averageTime: 28.7, trending: 'up' },
        { difficulty: 'Hard', solved: 44, total: 557, accuracy: 56.8, averageTime: 47.2, trending: 'down' }
      ],
      ...
      algorithmMastery: [
        { algorithm: 'Two Pointers', mastery: 85, problemsSolved: 23, lastUsed: '2 days ago', trending: 'up' },
        { algorithm: 'Sliding Window', mastery: 72, problemsSolved: 18, lastUsed: '1 day ago', trending: 'stable' },
        { algorithm: 'Binary Search', mastery: 68, problemsSolved: 15, lastUsed: '3 days ago', trending: 'up' },
        { algorithm: 'DFS', mastery: 79, problemsSolved: 31, lastUsed: '1 day ago', trending: 'stable' },
        { algorithm: 'Dynamic Programming', mastery: 45, problemsSolved: 12, lastUsed: '5 days ago', trending: 'down' },
        { algorithm: 'Greedy', mastery: 62, problemsSolved: 19, lastUsed: '2 days ago', trending: 'up' }
      ],
      competitiveRanking: {
        currentRank: 1543,
        totalUsers: 45632,
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track your coding journey and performance insights</p>
        </div>
      ...
      <div className="mt-6">
        ...
        {activeTab === 'categories' && <CategoriesTab data={analyticsData.categoryBreakdown} />}
        {activeTab === 'algorithms' && <AlgorithmsTab data={analyticsData.algorithmMastery} />}
        {activeTab === 'competitive' && <CompetitiveTab data={analyticsData.competitiveRanking} />}
      </div>
    </div>
  );
}
...
function MetricCard({ title, value, total, icon: Icon, color, suffix = '' }: MetricCardProps) {
  const colorClasses = {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {data.difficultyBreakdown.map(difficulty => (
          <div key={difficulty.difficulty} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h4 className="font-semibold text-gray-900">{difficulty.difficulty}</h4>
              <TrendIndicator trend={difficulty.trending} />
            </div>
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Solved:</span>
                <span className="font-medium">{difficulty.solved} / {difficulty.total}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{difficulty.averageTime} min</span>
              </div>
              <div className="bg-gray-200 rounded-full h-2">
                <div
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <h4 className="text-md font-medium text-gray-900 mt-6 mb-3">Improving</h4>
          <div className="space-y-3">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <h4 className="text-md font-medium text-gray-900 mt-6 mb-3">Stagnant</h4>
          <div className="space-y-3">
                ...
                <tr key={category.category} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {progress >= 80 ? (
                      ...
                      <span className="text-gray-600 font-medium">Beginner</span>
                    )}
                  </td>
                </tr>
              );
...
function AlgorithmsTab({ data }: { data: AlgorithmStats[] }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {data.map((algorithm) => (
        <div key={algorithm.algorithm} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-semibold text-gray-900">{algorithm.algorithm}</h4>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-sm font-medium text-gray-900">{algorithm.mastery}%</span>
              </div>
...
function CompetitiveTab({ data }: { data: RankingData }) {
  return (
    <div className="space-y-6">
      {/* Ranking Overview */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-6">Your Competitive Ranking</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-blue-600">#{data.currentRank}</div>
            <div className="text-sm text-gray-600">Current Rank</div>
            <div className={`text-xs mt-1 ${
              data.rankChange > 0 ? 'text-green-600' : data.rankChange < 0 ? 'text-red-600' : 'text-gray-500'
            }`}>
              {data.rankChange > 0 ? '+' : ''}{data.rankChange} from last week
            </div>
          </div>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-green-600">{data.percentile}%</div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">Nearby Rankings</h3>
        </div>
...
function TrendIndicator({ trend }: { trend: 'up' | 'down' | 'stable' }) {
  const icons = {
    ...
    down: <ArrowTrendingUpIcon className="h-4 w-4 text-red-500 transform rotate-180" />,
