...
const sampleExercises: RegexExercise[] = [
  {
    ...
    explanation: 'This pattern matches: username@domain.tld where username can contain letters, numbers, dots, underscores, plus signs, and hyphens. Domain must have at least one dot and a valid TLD.',
    difficulty: 'Beginner',
    category: 'Validation'
  },
  {
    ...
    explanation: 'This pattern matches US phone numbers in various formats including international format, parentheses, dots, spaces, and hyphens.',
  ...
  },
  {
    ...
    explanation: 'This pattern matches HTTP and HTTPS URLs, capturing the protocol and domain with optional path, query parameters, and fragments.',
    difficulty: 'Intermediate',
    category: 'Extraction'
  },
  {
    ...
    explanation: 'This pattern requires: at least 8 characters, one lowercase letter, one uppercase letter, one digit, and one special character.',
    difficulty: 'Advanced',
    category: 'Validation'
  }
]
...
export default function RegexPage() {
  const [exercises, setExercises] = useState<RegexExercise[]>(sampleExercises)
  const [selectedExercise, setSelectedExercise] = useState<RegexExercise | null>(null)
  ...
  const checkExercise = () => {
    ...
    try {
      ...
      const userMatches = selectedExercise.testStrings.map(str => userRegex.test(str))
      const correctMatches = selectedExercise.testStrings.map(str => correctRegex.test(str))
      
      const isCorrect = userMatches.every((match, index) => match === correctMatches[index])
      setIsValid(isCorrect)
      
      if (isCorrect) {
        setMatches(selectedExercise.testStrings.filter((_, index) => correctMatches[index]))
      }
    } catch (error) {
      setIsValid(false)
    }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <p className="mt-2 text-gray-600">Master regular expressions with interactive exercises</p>
        </div>
            ...
            {selectedExercise ? (
                ...
                <div className="px-4 py-5 sm:p-6">
                  ...
                  <div className="mb-6">
                    ...
                    {isValid !== null && (
                      <div className={`mt-2 p-2 rounded-md ${
                        ...
                        {isValid ? '✓ Pattern is correct!' : '✗ Pattern needs adjustment'}
                      </div>
                    )}
                  </div>
                  ...
                  {showSolution && (
                    <div className="mb-6 p-4 bg-blue-50 rounded-md">
                      ...
                      <p className="text-sm text-blue-700 mt-2">{selectedExercise.explanation}</p>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="bg-gray-50 p-3 rounded-md">
                        {matches.map((match, index) => (
                          <div key={index} className="text-sm text-gray-700 bg-white p-2 rounded mb-1">
                            {match}
                          </div>
                        ))}
                      </div>
              ...
              <div className="bg-white shadow rounded-lg">
                <div className="px-4 py-5 sm:p-6 text-center">
                  ...
                  <h3 className="mt-2 text-sm font-medium text-gray-900">Select an Exercise</h3>
                  <p className="mt-1 text-sm text-gray-500">Choose an exercise from the list to start practicing regex patterns.</p>
                </div>
              </div>
            )}
          </div>
