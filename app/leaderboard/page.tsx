...
export default function LeaderboardPage() {
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default:
        return <span className="text-lg font-bold text-gray-500 dark:text-slate-400">{rank}</span>;
    }
  };
  ...
  const currentData = leaderboardData[activeTab as keyof LeaderboardData] || leaderboardData.global;
            ...
            <div className="overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                ...
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                  {Array.isArray(currentData) && currentData.map((entry) => (
                    <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          <AcademicCapIcon className={`h-4 w-4 ${getLevelColor(entry.level)}`} />
