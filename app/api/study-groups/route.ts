...
async function joinChallenge(userId: string, params: any) {
  ...
  if (challenge.participants.some(p => p.userId === userId)) {
    return NextResponse.json({ error: 'Already joined challenge' }, { status: 400 });
  }
...
async function listStudyGroups(userId: string) {
  ...
  const myGroups = Array.from(studyGroups.values())
    ...
    .map(group => ({
      ...
      upcomingSessions: group.schedule.filter(s => 
...
function sanitizeGroupForUser(group: StudyGroup, userId: string) {
  const member = group.members.find(m => m.id === userId);
  const canViewPrivateInfo = member && (member.role === 'creator' || member.role === 'moderator');
  ...
  return {
    ...
    members: group.members.map(m => ({
      ...
      status: m.status,
      contributions: canViewPrivateInfo || m.id === userId ? m.contributions : undefined
    }))
  };
}

function generateChallengeRewards(type: string, difficulty: string, problemCount: number): ChallengeReward[] {
  const basePoints = problemCount * (difficulty === 'Easy' ? 10 : difficulty === 'Medium' ? 20 : 30);
...
function generateGroupRecommendations(userId: string): any[] {
...
async function leaveStudyGroup(userId: string, params: any) {
  ...
  if (group.creator === userId && group.members.length > 0) {
    const nextCreator = group.members.find(m => m.role === 'moderator') || group.members[0];
...
async function kickMember(userId: string, params: any) {
  ...
  if (!requester || (requester.role !== 'creator' && requester.role !== 'moderator')) {
    return NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 });
  }

  if (memberId === group.creator) {
    return NextResponse.json({ error: 'Cannot kick group creator' }, { status: 400 });
  }
...
async function promoteToModerator(userId: string, params: any) {
  ...
  if (group.creator !== userId) {
    return NextResponse.json({ error: 'Only creator can promote members' }, { status: 403 });
  }
...
async function inviteToGroup(userId: string, params: any) {
  ...
  const member = group.members.find(m => m.id === userId);
  if (!member || (!group.settings.allowMemberInvites && member.role === 'member')) {
    return NextResponse.json({ error: 'Insufficient permissions' }, { status: 403 });
  }
