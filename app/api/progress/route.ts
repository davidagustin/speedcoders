	...
	try {
      ...
      `)
		...
		const difficultyBreakdown = {
			easy: {
				solved: 0,
				total: problems?.filter((p) => p.difficulty === "Easy").length || 0,
			},
			medium: {
				solved: 0,
				total: problems?.filter((p) => p.difficulty === "Medium").length || 0,
			},
			hard: {
				solved: 0,
				total: problems?.filter((p) => p.difficulty === "Hard").length || 0,
			},
		};
