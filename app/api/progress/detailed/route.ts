    ...
    milestones: [
      {
        ...
        points: 100,
        unlockCriteria: { quizzesCompleted: 10, averageScore: 80, specificTopics: ['Array'] }
      },
      {
        ...
        points: 150,
        unlockCriteria: { quizzesCompleted: 5, averageScore: 75, specificTopics: ['Two Pointers'] }
      },
      {
        ...
        points: 200,
        unlockCriteria: { quizzesCompleted: 8, averageScore: 70, specificTopics: ['Sliding Window'] }
      }
    ],
    estimatedHours: 25,
    difficulty: 'Beginner'
    ...
    milestones: [
      {
        id: 'dp-intro',
        title: 'DP Foundation',
        description: 'Understand basic DP concepts and solve 5 classic problems',
        requiredSkills: ['Dynamic Programming', 'Recursion'],
        points: 250,
        unlockCriteria: { quizzesCompleted: 5, averageScore: 60, specificTopics: ['Dynamic Programming'] }
      },
      {
        id: 'dp-optimization',
        ...
        points: 350,
        unlockCriteria: { quizzesCompleted: 10, averageScore: 70, specificTopics: ['Dynamic Programming'] }
      }
    ],
    estimatedHours: 40,
    difficulty: 'Advanced'
...
export async function GET(request: Request) {
  try {
    ...
    const recommendations = generateRecommendations(skillProgress, pathProgress, attempts);
...
function calculateSkillProgress(attempts: any[]) {
  ...
  attempts.forEach(attempt => {
    ...
    if (category) {
      ...
      skillStats[category].bestScore = Math.max(skillStats[category].bestScore, attempt.score);
...
function calculateLearningPathProgress(attempts: any[], skillProgress: Record<string, any>) {
  return learningPaths.map(path => {
    ...
    const masteredSkills = requiredSkills.filter(skill => 
      skillProgress[skill]?.proficiencyLevel === 'Expert' || 
      skillProgress[skill]?.proficiencyLevel === 'Advanced'
    );

    const nextMilestone = path.milestones.find(milestone => 
      !isMilestoneCompleted(milestone, attempts, skillProgress)
    );
    ...
    return {
      ...
      estimatedTimeRemaining: calculateTimeRemaining(path, completionPercentage)
    };
  });
}

function isMilestoneCompleted(milestone: Milestone, attempts: any[], skillProgress: Record<string, any>): boolean {
  ...
  if (criteria.quizzesCompleted) {
    const relevantAttempts = attempts.filter(attempt => 
      ...
      criteria.specificTopics.includes(attempt.quiz?.category) : true
    );
    if (relevantAttempts.length < criteria.quizzesCompleted) return false;
  }

  if (criteria.averageScore && criteria.specificTopics) {
    const topicScores = criteria.specificTopics.map(topic => 
      skillProgress[topic]?.averageScore || 0
    );
...
function isPathUnlocked(path: LearningPath, skillProgress: Record<string, any>): boolean {
  if (path.difficulty === 'Beginner') return true;
  if (path.difficulty === 'Intermediate') {
    return Object.values(skillProgress).some((skill: any) => 
      skill.proficiencyLevel === 'Intermediate' || skill.proficiencyLevel === 'Advanced' || skill.proficiencyLevel === 'Expert'
    );
  }
  ...
  if (path.difficulty === 'Expert') {
    return Object.values(skillProgress).filter((skill: any) => 
...
function calculateTimeRemaining(path: LearningPath, completionPercentage: number): number {
  return Math.max(0, Math.round(path.estimatedHours * (100 - completionPercentage) / 100));
}

function calculateAchievements(attempts: any[], skillProgress: Record<string, any>) {
  const achievements = [];
  ...
  const expertSkills = Object.keys(skillProgress).filter(skill => 
    skillProgress[skill].proficiencyLevel === 'Expert'
  );
...
function generateRecommendations(skillProgress: Record<string, any>, pathProgress: any[], attempts: any[]) {
  const recommendations = [];

  // Identify weak areas
  const weakSkills = Object.entries(skillProgress)
    .filter(([_, stats]: [string, any]) => stats.averageScore < 60)
    .sort(([_, a]: [string, any], [__, b]: [string, any]) => a.averageScore - b.averageScore)
    .slice(0, 3);
  ...
  const availablePaths = pathProgress.filter(path => 
    path.isUnlocked && path.completionPercentage < 100
  ...
  const recentAverage = recentAttempts.reduce((sum, a) => sum + a.score, 0) / recentAttempts.length;
  ...
  if (recentAverage >= 80) {
    recommendations.push({
      ...
      description: 'Your recent performance suggests you can handle more difficult challenges',
