...
export async function POST() {
  try {
    ...
    const algorithms = [
      {
        ...
        examples: JSON.stringify([
          { problem: 'Two Sum', approach: 'Use hash table to store complements' },
      ...
      },
      {
        ...
        examples: JSON.stringify([
          ...
          { problem: 'Container With Most Water', approach: 'Move pointer with smaller height' }
        ])
      },
      {
        name: 'Sliding Window',
        ...
        spaceComplexity: 'O(1)',
        examples: JSON.stringify([
          { problem: 'Longest Substring Without Repeating Characters', approach: 'Expand window until duplicate found' },
          { problem: 'Minimum Size Subarray Sum', approach: 'Shrink window when sum >= target' }
        ])
      },
      {
        ...
        examples: JSON.stringify([
          ...
          { problem: 'Coin Change', approach: 'dp[amount] = min(dp[amount - coin] + 1)' }
        ])
      },
      {
        name: 'Binary Search',
        ...
        examples: JSON.stringify([
          ...
          { problem: 'Search in Rotated Sorted Array', approach: 'Handle rotation with modified binary search' }
        ])
      },
      {
        ...
        spaceComplexity: 'O(V)',
        examples: JSON.stringify([
          { problem: 'Number of Islands', approach: 'DFS to mark connected land' },
          { problem: 'Validate Binary Search Tree', approach: 'Inorder traversal should be sorted' }
        ])
      },
      {
        ...
        spaceComplexity: 'O(V)',
        examples: JSON.stringify([
          { problem: 'Binary Tree Level Order Traversal', approach: 'Use queue for level-by-level traversal' },
          { problem: 'Word Ladder', approach: 'BFS to find shortest transformation sequence' }
        ])
      },
      {
        name: 'Stack',
        ...
        spaceComplexity: 'O(n)',
        examples: JSON.stringify([
          { problem: 'Valid Parentheses', approach: 'Push opening brackets, pop on closing' },
          { problem: 'Min Stack', approach: 'Maintain minimum value alongside stack' }
        ])
      }
    ];
    ...
    const problems = [
      ...
      },
      {
        ...
        examples: JSON.stringify([
          ...
          { input: 's = "()[]{}"', output: 'true', explanation: 'Multiple valid parentheses.' },
          { input: 's = "(]"', output: 'false', explanation: 'Mismatched parentheses.' }
        ]),
        constraints: JSON.stringify([
          '1 <= s.length <= 104',
    ...
    const learningPaths = [
      {
        ...
        description: 'Master essential data structures from arrays to advanced trees',
        category: 'beginner',
        difficulty: 'easy',
        duration: 20,
    ...
    const systemDesigns = [
      {
        ...
        description: 'Design a service that takes long URLs and converts them into shorter URLs',
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Database seeded successfully with algorithms, problems, learning paths, and system designs' 
    });
