...
export default function QuizPage({ params }: QuizPageProps) {
		...
		const fetchQuiz = async () => {
			try {
				...
				if (response.ok) {
					...
					const quizProblems = quizData.problemIds
						.map((id: string) =>
							allProblems.find((p) => p.id === parseInt(id, 10)),
						)
						.filter(Boolean);
	...
	}, [timeLeft, isPaused, isSubmitted, handleSubmitQuiz]);
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center space-x-4">
							...
							<div className="flex items-center space-x-2">
								...
								<span
									className={`font-mono text-lg ${timeLeft < 300 ? "text-red-600" : "text-gray-900"}`}
								>
									{formatTime(timeLeft)}
								</span>
							</div>
								...
								{currentProblem?.algorithms.map((algorithm: string) => (
									...
									>
										<input
											type="checkbox"
											checked={
												selectedAlgorithms[currentProblem.id]?.includes(
													algorithm,
												) || false
											}
											onChange={() =>
												handleAlgorithmSelection(currentProblem.id, algorithm)
											}
									...
									<button
										onClick={() =>
											setCurrentProblemIndex(
												Math.max(0, currentProblemIndex - 1),
											)
										}
