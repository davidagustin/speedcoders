...
export default function CreateQuizPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Please sign in to create a quiz</h1>
  ...
  const algorithmOptions = [
    "Two Pointers", "Sliding Window", "Hash Table", "Prefix Sum",
    "Binary Search", "Sorting", "Greedy", "Dynamic Programming",
    "Backtracking", "Tree Traversal", "Graph Traversal", "Union Find",
    "Topological Sort", "Trie", "Segment Tree", "Binary Indexed Tree",
    "Stack", "Queue", "Linked List", "Heap"
  ];

  const filteredProblems = problems.filter(problem => {
    const matchesDifficulty = difficulty === "all" || problem.difficulty === difficulty;
    const matchesAlgorithm = algorithm === "all" || problem.algorithms.includes(algorithm);
  ...
  const handleQuickStart = (type: string) => {
    ...
    if (type === "random") {
      ...
      quickProblems = shuffled.slice(0, 5).map(p => p.id);
    } else if (type === "easy") {
              ...
              <div className="p-6 border-b">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-600">{timeLimit} min timer</span>
                  </div>
                </div>
              </div>
              ...
              <div className="divide-y divide-gray-200 max-h-96 overflow-y-auto">
                {filteredProblems.length === 0 ? (
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm">Try adjusting your difficulty or algorithm selection.</p>
                  </div>
                ) : (
                          ...
                          <div className="flex items-center mt-2 space-x-2">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {problem.algorithms.slice(0, 3).map(algo => (
                                <span key={algo} className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-800">
                                  {algo}
                                </span>
                              ))}
