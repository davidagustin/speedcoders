...
export function PerformanceTracker({ enabled = true, showDebugPanel = false }: PerformanceTrackerProps) {
    ...
    <div className="fixed bottom-4 right-4 z-50">
      ...
      {isVisible && (
        <div className="bg-black bg-opacity-90 text-white p-4 rounded-lg max-w-md max-h-96 overflow-y-auto shadow-2xl">
          ...
          <div className="mb-4">
            <h4 className="font-semibold mb-2 text-yellow-400">Core Web Vitals</h4>
                ...
                return (
                  <div key={key} className="flex justify-between">
                    ...
                    <span className={statusColors[status]}>
                      {key === 'cls' ? (value as number).toFixed(3) : formatValue(value as number)}
                    </span>
                  </div>
                )
              })}
            ...
            <div className="space-y-1 text-xs max-h-32 overflow-y-auto">
              {metrics.slice(-10).reverse().map((metric, index) => (
                <div key={index} className="flex justify-between">
                  ...
                  <span className="text-gray-300">{formatValue(metric.value)}</span>
                </div>
              ))}
            </div>
          </div>
