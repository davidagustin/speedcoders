...
import { QuizQuestion, Quiz } from '@/lib/QuizManager'
...
export function QuizCreator({ onQuizCreated, onCancel }: QuizCreatorProps) {
  ...
  const categories = [
    ...
    'Stack', 'Queue', 'Hash Table', 'Linked List', 'Sorting'
  ]

  const algorithms = [
    'Two Pointers', 'Sliding Window', 'Binary Search', 'DFS', 'BFS',
    'Kadane\'s Algorithm', 'Dijkstra', 'Union Find', 'Backtracking'
  ]
  ...
  const validateQuiz = (): string[] => {
    ...
    if (quiz.categories.length === 0) errors.push('At least one category is required')
    if (quiz.timeLimit < 5) errors.push('Time limit must be at least 5 minutes')
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
              ...
              {categories.map(category => (
                <label key={category} className="flex items-center gap-1 text-sm">
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setQuiz({ ...quiz, categories: [...quiz.categories, category] })
                      } else {
                        setQuiz({ ...quiz, categories: quiz.categories.filter(c => c !== category) })
                      }
                    }}
                    className="rounded"
                  />
      ...
      <div className="space-y-6">
        ...
        {questions.map((question, questionIndex) => (
          <div key={questionIndex} className="bg-white p-6 rounded-lg shadow border">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-md font-semibold">Question {questionIndex + 1}</h4>
              {questions.length > 1 && (
                <button
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={(e) => updateQuestion(questionIndex, 'algorithms', Array.from(e.target.selectedOptions, option => option.value))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    size={3}
                  >
                    {algorithms.map(alg => (
