...
export function StudyPlanner({
  ...
  const [activeTab, setActiveTab] = useState<'calendar' | 'goals' | 'analytics'>('calendar')
...
function CalendarView({
  ...
  const getTypeColor = (type: StudySession['type']) => {
    switch (type) {
      case 'problems': return 'bg-blue-100 text-blue-800'
        ...
        <div>
          ...
          <div className="space-y-3">
            {completedSessions.slice(0, 5).map(session => (
              <div key={session.id} className="bg-green-50 rounded-lg p-4 border border-green-200">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
...
function AnalyticsView({ sessions, goals }: { sessions: StudySession[], goals: StudyGoal[] }) {
  const completedSessions = sessions.filter(s => s.completed)
  const totalStudyTime = completedSessions.reduce((sum, s) => sum + (s.actualDuration || s.duration), 0)
  const averageSessionDuration = completedSessions.length > 0 ? totalStudyTime / completedSessions.length : 0
  
  const completedGoals = goals.filter(g => g.completed)
  const activeGoals = goals.filter(g => !g.completed)
  const avgGoalProgress = activeGoals.length > 0 
    ? activeGoals.reduce((sum, g) => sum + (g.current / g.target), 0) / activeGoals.length * 100
    : 0
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="bg-gray-50 rounded-lg p-6">
          ...
          <p className="text-sm text-gray-600">Keep it up! You're on a great streak.</p>
        </div>
      </div>
...
function SessionModal({
  ...
  const [formData, setFormData] = useState({
    ...
    scheduledDate: session?.scheduledDate ? session.scheduledDate.split('T')[0] : new Date().toISOString().split('T')[0],
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, difficulty: e.target.value as StudySession['difficulty'] })}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                value={formData.unit}
                onChange={(e) => setFormData({ ...formData, unit: e.target.value as StudyGoal['unit'] })}
                className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="problems">Problems</option>
